...
public class KapitalService implements CommonService {
    ...
    public ResponseDto<List<Currency>> getCurrencies(Instant date) {
        ...
        log.debug("check if list is empty or not");
        if (!list.isEmpty()) {
            log.debug("list is not empty");
//            List<CurrencyDTO> dto = currencyMapper.toDto(list);
